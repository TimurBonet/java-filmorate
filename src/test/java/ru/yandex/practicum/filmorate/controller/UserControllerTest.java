...
class UserControllerTest {
    ...
    public void beforeEach() {
        User user1 = new User("1","abc1@mail.ru", "cba", LocalDate.parse("1993-04-15"));
        ...
        User user6 = new User(1, "art@mail.com","asdegfr","name",LocalDate.parse("1989-12-22"));
        usersList.add(user1);
        usersList.add(user2);
        usersList.add(user3);
        usersList.add(user4);
...
/*User user = usersList.get(1);
        ...
        assertEquals(user, userController.getAllUsers().get(0), "Пользователь не совпадает");
        assertEquals(user.getId(), userController.getAllUsers().get(0).getId(), "Не совпадает id");
        assertEquals("NoName", userController.getAllUsers().get(0).getName(), "Присвоенное имя не совпадает");
    }
    ...
    void updateUser() {
        User user = usersList.get(0);
        userController.createUser(user);
        User user2 = userController.getAllUsers().get(0);
        ...
        assertEquals(1, userController.getAllUsers().size(), "Количество пользователей не совпадает");
        assertEquals(user2, userController.getAllUsers().get(0), "Пользователь не обновлён");
    }
    ...
    void shouldShowCommonFriendList() {
        ...
        User expected = user2;
        assertEquals(expected, userController.getCommonFriendList(id1, id2).get(0), "Списки друзей не совпадают");
    }
}*/
